...
class Hash {
    ...
    public function encrypt($bvn, $aes_key, $ivkey) {
        $EncryptedText = openssl_encrypt($bvn, "AES-128-CBC", $aes_key, $options = OPENSSL_RAW_DATA, $ivkey);
        return bin2hex($EncryptedText);
    }

    public function decrypt($data, $aes_key, $ivkey){
        $result = openssl_decrypt(hex2bin($data), "AES-128-CBC", $aes_key, $options=OPENSSL_RAW_DATA|OPENSSL_ZERO_PADDING, $ivkey);
